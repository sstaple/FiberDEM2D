...
namespace PlotFDEM
{
    ...
    public class ReadCSV
    {
                ...
                if (temp[0].Contains("*")) { //
                    ...
                    switch (sSection) {
                        ...
                        case "*Fibers":
                            ...
                            double p1x = Convert.ToDouble(temp[2]);
                            ...
                            if (currIndex != -1) { //If the fiber is the original
                                ...
                                lFibers[nFiber].Add(p1x, p1y, p1z, r, l, rot, isProjected, true);
                                lProjFibers[nFiber][0].Add(0, 0, 0, r, l, rot, true, false);
                                lProjFibers[nFiber][1].Add(0, 0, 0, r, l, rot, true, false);
                                lProjFibers[nFiber][2].Add(0, 0, 0, r, l, rot, true, false);
                                nFiber++;
                                nPFiber = 0;
                            }
                        ...
                        case "*Contacts":
                            ...
                            ReadContacts(iteration, ref nContact, ref nSizing, temp, tempLine);
                            break;
                            #endregion
        ...
        virtual protected void ReadContacts(int iteration, ref int nContact, ref int nSizing, string[] temp, string fullLine)
        {
            ...
            {
                //Find the coordinate
                double P1y = f1.lPositions[iteration][1];
                double P1z = f1.lPositions[iteration][2];
                double P2y = f2.lPositions[iteration][1];
                double P2z = f2.lPositions[iteration][2];
            ...
            else if (temp[4] == "MatrixModel1")
            {
                matrixModel = new MatrixContinuumModel1(temp[4], fullLine);
                lMatrix.Add(new MatrixContinuum.MatrixContinuum(f1, f2, matrixModel));
                ...
                foreach (MatrixContinuum.MatrixContinuum matrixContinuum in lMatrix)
                {
                    if (matrixContinuum.nf1 == fIndex1 && matrixContinuum.nf2 == fIndex2)
                    {
                        ...
                        else if (matrixModel is MatrixContinuumElasticFiberDamageModel)
                        {
                            ...
                            matrixContinuum.Add(q, Damage, iteration, true, true);
                        }

                        else if (matrixModel is MatrixContinuumModel1)
                        {
                            double[] q = new double[] {Convert.ToDouble(temp[5]), Convert.ToDouble(temp[6]) , Convert.ToDouble(temp[7]) , Convert.ToDouble(temp[8]),
                        Convert.ToDouble(temp[9])};

                            //Get the state variables
                            double[] Damage = new double[temp.Length - 10];

                            for (int i = 0; i < Damage.Length; i++)
                            {
                                Damage[i] = Convert.ToDouble(temp[i + 10]);
                            }
                            matrixContinuum.Add(q, Damage, iteration, true, false);
                        }
                    }
                }
            }
    ...
    public class Fiber {
        ...
        public void Add(double inX, double inY, double inZ, double inRadii, double inLength, double inRotation, bool inIsBorder, bool inIsVisible) {
            lPositions.Add(new double[3] {inX, inY, inZ });
        ...
        public void Add(double inX, double inY, double inZ, double inRadii, double inLength, double inRotation, bool inIsBorder, bool inIsVisible, bool overwriteLastIndex) {
            lPositions[lPositions.Count - 1] = new double[3] { inX, inY, inZ };
        ...
        public void DrawVelocity(Graphics graphic, int nTimeStep)
        {
            if (nTimeStep > 0) {
                if (!lIsBorder[nTimeStep]) { //For now, don't draw borders because they can switch with proj.  To change add projections to the fiber
                    ...
                    graphic.DrawLine(fiberBrush, (float)lPositions[nTimeStep][1], (float)lPositions[nTimeStep][2], (float)pt2[1], (float)pt2[2]);
        ...
        public void DrawOrientation(Graphics graphic, int nTimeStep)
        {
            ...
            if (lIsVisible[i])
            {
				...
				graphic.DrawLine(mypen, (float)lPositions[i][1], (float)lPositions[i][2], (float)(lPositions[i][1]+ rotUp[0]), (float)(lPositions[i][2]+ rotUp[1]));
				mypen.Dispose();
            }
        ...
        public void DrawFiberNumbers(Graphics graphic, int nTimeStep, Color color)
        {
            ...
            if (lIsVisible[i])
            {
                ...
                graphic.DrawString(fiberIndex.ToString(), font, brush, (float)(lPositions[i][1] - lRadii[i] / 4.0), -(float)(lPositions[i][2] + lRadii[i] / 4.0));
                brush.Dispose();
        ...
        public void Draw(Graphics graphic, int i, Color objectColor) {
            ...
            if (lIsVisible[i]) {
                ...
                graphic.FillEllipse(fiberBrush, (float)(lPositions[i][1] - lRadii[i]), (float)(lPositions[i][2] - lRadii[i]), (float)(2.0 * lRadii[i]), (float)(2.0 * lRadii[i]));
                fiberBrush.Dispose();
