	...
	public class ContourPlotMaker : Animation.IDrawGraphic
    {
        ...
        public bool AutomaticRange
        {
            ...
            set { bAutomaticRange = value;
                bChangeRange = bAutomaticRange ? true : bChangeRange;
                //Only change the range if it is already automatic range
            }
        }
        ...
        public void Draw(Graphics graphic, int i)
        {
            ...
            if (bShowProjections)
            {
                ...
                PointF ul = Subtract(corners[1], corners[0]);
                PointF ur = Subtract(corners[2], corners[0]);
                PointF lr = Subtract(corners[3], corners[0]);
                
                //Right
                DrawTranslated(graphic, i, lr.X, lr.Y);
                //Left
                DrawTranslated(graphic, i, -1.0f * lr.X, -1.0f * lr.Y);
                //Top
                DrawTranslated(graphic, i, ul.X, ul.Y);
                //Bottom
                DrawTranslated(graphic, i, -1.0f * ul.X, -1.0f * ul.Y);
                //BottomLeft
                DrawTranslated(graphic, i, -1.0f * ur.X, -1.0f * ur.Y);
                //TopRight
        ...
        public void Update()
        {
            ...
            if (bChangeLevels || bChangeRange || (bChangePlotType && bAutomaticRange))
            {
                CreateLevels();
            }

            //Create new colors if needed
            if (bChangeLevels || bChangeRange || bChangeColor || (bChangePlotType && bAutomaticRange))
            {
                CreateLevelColors();
            }
		...
		public int[] FindMaxMinAndOutputAscendingOrder(ref double max, ref double min)
        {
            ...
            for (int i = 0; i < lMatrix.Count; i++)
            {
                ...
                tempMax = tempMin = 0.0;
                lMatrix[i].FindMaxAndMinResults(0, ref tempMax, ref tempMin);
