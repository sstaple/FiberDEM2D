...
namespace PlotFDEM.MatrixContinuum
{
    ...
    public class MatrixContinuum
    {
...

        {
            ...
            double rotation = Math.Atan2(x12[2], x12[1]);
            ...
            transform.Translate((float)(xf1[1]), (float)(xf1[2]));
            ...
            //Save Results
            double di = VectorMath.Norm(new double[] { x12[1], x12[2] });
            ld12.Add(di);
            lq.Add(q);
        ...
        /// <param name="index"></param>
        public virtual void Add(double[] q, double[] damage, int index, bool hasDamage, bool isFullQ)
        {
            if (!isFullQ)
            {
                MatrixContinuumModel1 mat = (MatrixContinuumModel1)matrixModel;
                mat.AddDamageAndTotalQ(damage, q);
                ...
                for (int i = 0; i < nGridPts + 1 ; i++)
                {
                    for (int j = 0; j < nGridPts + 1; j++)
                    {
                        double topResult = DecideOnOutput(nPlotType, nPlotComponent, b, (double)topPoints[currIndex][i, j].point.X,
                            (double)topPoints[currIndex][i, j].point.Y, lq[currIndex], currIndex, true);
                        double bottomResult = DecideOnOutput(nPlotType, nPlotComponent, b, (double)bottomPoints[currIndex][i, j].point.X,
                            (double)bottomPoints[currIndex][i, j].point.Y, lq[currIndex], currIndex, true);
        ...
        public void DrawConnections(int nTimeStep, Graphics panelGraphic)
        {
            ...
            if (lIteration.Contains(nTimeStep))
            {
                ...
                panelGraphic.DrawLine(fiberBrush, (float)f1.lPositions[nTimeStep][1], (float)f1.lPositions[nTimeStep][2], 
                    (float)f2.lPositions[nTimeStep][1], (float)f2.lPositions[nTimeStep][2]);
            ...
            bool bPlotMatrixResultsOnly=false)
        {
            ...
            double yLeft = this.CalculateYAtLeftFiber(z);
            double yRight = this.CalculateYAtRightFiber(z);

            switch (nPlotType)
            {
                case 0: //local stress
                    double[] tempStress = matrixModel.CalculateStress(x, y, z, q, iteration, yLeft, yRight, bPlotMatrixResultsOnly);
                ...
                case 1: //global stress
                    double[] tempGStress = matrixModel.CalculateStress(x, y, z, q, iteration, yLeft, yRight, bPlotMatrixResultsOnly);
                    tempGStress = MatrixMath.Multiply(lglobalS_rotationTransform[iteration], tempGStress);
                    switch (nPlotComponent)
                    {
                        case 0:
                ...
                case 2: //local strain
                    double[] tempstrain = matrixModel.CalculateStrain(x, y, z, q, iteration, yLeft, yRight, bPlotMatrixResultsOnly);
                    switch (nPlotComponent)
                    {
                        case 0:
                ...
                case 3: //global strain
                    double[] tempgstrain = matrixModel.CalculateStrain(x, y, z, q, iteration, yLeft, yRight, bPlotMatrixResultsOnly);
                    tempgstrain = MatrixMath.Multiply(lglobalS_rotationTransform[iteration], tempgstrain);
                    switch (nPlotComponent)
                    {
                        case 0:
                ...
                case 4: //local displacement
                    double[] tempU = matrixModel.CalculateDisplacement(x, y, z, q, iteration, yLeft, yRight, bPlotMatrixResultsOnly);
                ...
                case 5: //global displacement
                    double[] tempU2 = matrixModel.CalculateDisplacement(x, y, z, q, iteration, yLeft, yRight, bPlotMatrixResultsOnly);
                    tempU2 = MatrixMath.Multiply(lglobalQ_rotationTransform[iteration], tempU2);
                ...
                case 6: //Damage
                    double D = matrixModel.CalculateDamage(x, y, z, q, iteration, yLeft, yRight, bPlotMatrixResultsOnly);
    ...
    {
        double[] CalculateDisplacement(double x, double y, double z, double[] q, int iteration, double yLeft, double yRight,
            bool bPlotMatrixResultsOnly = false);
        double[] CalculateStrain(double x, double y, double z, double[] q, int iteration, double yLeft, double yRight,
            bool bPlotMatrixResultsOnly = false);
        double[] CalculateStress(double x, double y, double z, double[] q, int iteration, double yLeft, double yRight,
            bool bPlotMatrixResultsOnly = false);
        double CalculateDamage(double x, double y, double z, double[] q, int iteration, double yLeft, double yRight,
            bool bPlotMatrixResultsOnly = false);
