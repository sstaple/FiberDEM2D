...
namespace FDEMCore.Contact
{
    ...
    /// </summary>
    public class Matrix_ElasticFiber_Coupled : IMatrixModel
    {
        ...
        //Stiffness matrix for the fibers and matrix
        ...
        //This is for the guyan reduction, where the matrix dof's are condensated out
        public double[,] Kff;
        public double[,] Kmm;
        public double[,] Kmf;
        public double[,] Kfm;
        ...
        #region Constructor
        public Matrix_ElasticFiber_Coupled(MatrixAssemblyParameters matrix, Fiber fiber1, Fiber fiber2, double dCoeff) 
        {
            //TODO: fix the constructor call: just added this to remove errors.
            isImplicit = false;
        ...
        public Matrix_ElasticFiber_Coupled(double r, double d, double b, double Ep, double nu, Fiber fiber1, Fiber fiber2)
        {
            this.b = b;
            this.d = d;
            this.nu = nu;
        ...
        public virtual double[] CalculateStress(double x, double y, double z, double[] qm)
        {
            ...
            return MatrixMath.Multiply(Dm, strain);
        }
       ...
       a2 * (5.0 * d2 - 4.0 * d2 * nu + 4.0 * r2 - 8.0 * nu * r2))) / (12.0 * c1 * d);
        ...
        4.0 * (l3 * r2 + l4 * r2 + l5 * r2 - l9 * r2 - 2.0 * l3 * nu * r2 - 2.0 * l4 * nu * r2 -
           ...
           l11 * (-1.0 + 2.0 * nu) * r2 + 6.0 * d * rmz - 6 * d * nu * rmz + 6 *  d * nu * rmz))) / (48.0 * d);
            ...
            Complex k44 = (b * Ep * (a1 * c1 * d * (d2 + 3.0 * (-2.0 + nu -  nu) * r2) - 2.0 * a4 * c1 * r *
        (2.0 * d2 * (-1.0 + nu) + (-1.0 + 2 * nu) * r2) -
       a2 * (d4 - 6.0 * d2 * r2 + 4.0 * d2 * nu * r2 - 4.0 * r4 + 8.0 * nu * r4) -
       a3 * (d4 - 6.0 * d2 * r2 + 4.0 * d2 * nu * r2 - 4.0 * r4 + 8.0 * nu * r4) +
       3.0 * c1 * d * (2.0 + (-1.0) * nu) * rmz * z)) /
   (12.0 * c1 * d);


            Complex k48 = (b * Ep * (a1 * c1 * d * (d2 * (-3.0 + 4.0 * nu) + 6.0 * (2.0 + (-1.0) * nu) * r2) +
       4.0 * a4 * c1 * r * (d2 * (-1.0 + nu) + (-1.0 + 2.0 * nu) * r2) -
       a2 * (-3.0 * d4 + 4.0 * d4 * nu + 4.0 * d2 * r2 + 8.0 * r4 - 16.0 * nu * r4) -
       a3 * (-3.0 * d4 + 4.0 * d4 * nu + 4.0 * d2 * r2 + 8.0 * r4 - 16.0 * nu * r4) - 6.0 * c1 * d * (2.0 + (-1.0) * nu) * rmz * z))
    / (24.0 * c1 * d);
